...
class Graph(object):
        ...
        while len(queue) > 0:
            ...
            for out_node in self.edges[node.node_id]:
                if out_node.asap_schd is None or out_node.asap_schd < (
                    node.asap_schd + node.fwd_latency + node.fwd_comm_latency
                ):
                    out_node.asap_schd = (
                        node.asap_schd + node.fwd_latency + node.fwd_comm_latency
                    )
        ...
        while len(queue) > 0:
            ...
            for in_node in self.in_edges[node.node_id]:
                if in_node.alap_schd is None or in_node.alap_schd < (
                    node.sink_latency + in_node.fwd_latency
                ):
                    in_node.sink_latency = node.sink_latency + in_node.fwd_latency
                if in_node.node_id not in visited:
                    queue.append(in_node)
    ...
    def populate_alap_schd(self):
        ...
        while len(queue) > 0:
            ...
            for in_node in self.in_edges[node.node_id]:
                if in_node.alap_schd is None or in_node.alap_schd > (
                    node.alap_schd - in_node.fwd_latency - in_node.fwd_comm_latency
                ):
                    in_node.alap_schd = (
                        node.alap_schd - in_node.fwd_latency - in_node.fwd_comm_latency
                    )
