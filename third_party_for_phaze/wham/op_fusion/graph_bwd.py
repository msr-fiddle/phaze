...
class Graph(object):
    ...
    def populate_depths(self):
        # Helper method that annotates each node in the graph with its depth from the source.
        ...
        while len(queue) > 0:
            ...
            for out_node in self.edges[node.node_id]:
                ...
                if (
                    self.check_all_predecessors_depth(out_node, visited)
                    and out_node.node_id not in visited
                ):
                    queue.append(out_node)

    def check_all_predecessors_depth(self, node, visited):
        valid_node = True
        pred_nodes = self.get_predecessors(node)
    ...
    def populate_asap_schd(self):
        # Helper method that annotates each node in the graph with its asap schedule.
        sources = self.sources()
        sources[0].asap_schd = 0
        queue = [sources[0]]
        visited = set()
        while len(queue) > 0:
            node = queue.pop(-1)
            ...
            if node.node_id not in self.edges:
                continue
            for out_node in self.edges[node.node_id]:
                if out_node.asap_schd is None or out_node.asap_schd < (
                    node.asap_schd
                    + node.bwd_latency
                    + node.bwd_comm_latency
                    + node.weight_update_latency
                ):
                    out_node.asap_schd = (
                        node.asap_schd
                        + node.bwd_latency
                ...
                if (
                    self.check_all_predecessors_asap_schd(out_node, visited)
                    and out_node.node_id not in visited
                ):
                    queue.append(out_node)

    def check_all_predecessors_asap_schd(self, node, visited):
        valid_node = True
        pred_nodes = self.get_predecessors(node)
        for node in pred_nodes:
            if node.node_id not in visited:
    ...
    def populate_sink_latency(self):
        # Helper method that annotates each node in the graph with its latency from the sink.
        sinks = self.sinks()
        for sink in sinks:
            sink.sink_latency = sink.fwd_latency
        queue = sinks
        visited = set()
        while len(queue) > 0:
            node = queue.pop(-1)
            ...
            if node.node_id not in self.in_edges:
                continue
            for in_node in self.in_edges[node.node_id]:
                if in_node.alap_schd is None or in_node.alap_schd < (
                    node.sink_latency
                    + in_node.bwd_latency
                    + in_node.weight_update_latency
                ):
                    in_node.sink_latency = (
                        node.sink_latency
                        + in_node.bwd_latency
                        + in_node.weight_update_latency
                    )
    ...
    def populate_alap_schd(self):
        # Helper method that annotates each node in the graph with its alap schedule.
        sinks = self.sinks()
        sinks[0].alap_schd = sinks[0].asap_schd
        queue = [sinks[0]]
        visited = set()
        while len(queue) > 0:
            node = queue.pop(-1)
            ...
            for in_node in self.in_edges[node.node_id]:
                ...
                if (
                    self.check_all_successors_alap_schd(in_node, visited)
                    and in_node.node_id not in visited
                ):
                    queue.append(in_node)

    def check_all_successors_alap_schd(self, node, visited):
        valid_node = True
        succ_nodes = self.get_successors(node)
    ...
    def populate_mobility(self):
        nodes = list(self.nodes.values())
        for node in nodes:
            node.mobility = node.alap_schd - node.asap_schd
    ...
    def compress_branch_helper(self, node, new_node_id):
        if len(self.in_edges[node.node_id]) > 1:
            return None, node
        new_node = Node(
            "compressed_node%d" % new_node_id, node_desc=("Branch %d" % new_node_id)
        )
        chain_length = 0
        # Assumption here is that node has edges coming into it, since this is how
        # compress_branch_helper was called on it.
        while (
            len(self.in_edges[node.node_id]) == 1
            and node.node_id in self.edges
            and len(self.edges[node.node_id]) == 1
        ):
            chain_length += 1
            next_node = self.edges[node.node_id][
                0
            ]  # Since node has a single out-neighbor.
    ...
    def compress_branches(self):
        # Start exploration with the input graph's source node.
        nodes = self.sources()
                ...
                for out_node in self.edges[node.node_id]:
                    ...
                    if compressed_node is None:
                        ...
                        if node.node_id in new_node_mapping:
                            new_gr.add_edge(
                                new_node_mapping[node.node_id], out_node)
                        else:
                        ...
                        if node.node_id in new_node_mapping:
                            new_gr.add_edge(
                                new_node_mapping[node.node_id], compressed_node
                            )
                        else:
                ...
                if node.node_id in self.edges:
                    for out_node in self.edges[node.node_id]:
                        ...
                        if out_node.node_id in new_node_mapping:
                            new_gr.add_edge(
                                in_node, new_node_mapping[out_node.node_id])
                        else:
    ...
    def chain_nodes(self):
        ...
        for node in self.nodes.values():
            if (
                node.node_id in self.edges
                and len(self.edges[node.node_id]) == 1
                and node.node_id in self.in_edges
                and len(self.in_edges[node.node_id]) == 1
            ):
    ...
    def aggregate(self, sum_activations=False):
        ...
        for node in self.nodes.values():
            ...
            if sum_activations:
                ...
                if (
                    node.node_id not in self.in_edges
                    or len(self.in_edges[node.node_id]) == 0
                ):
                    activation_size += node.activation_size
        return [
    ...
    def check_isomorphism(self, other):
        ...
        for (self_node, other_node) in zip(
            self_topological_sort, other_topological_sort
        ):
            assert self_node.node_desc == other_node.node_desc
            if self_node.node_id in self.edges:
                assert len(self.edges[self_node.node_id]) == len(
                    other.edges[other_node.node_id]
                )
            if self_node.node_id in self.in_edges:
                assert len(self.in_edges[self_node.node_id]) == len(
                    other.in_edges[other_node.node_id]
                )
    ...
    def get_predecessors(self, node):
        return self.in_edges[node.node_id]
        ...
        for antichain_node in antichain:
            ...
            for predecessor in predecessors:
                for out_node in self.edges[predecessor.node_id]:
                    if (
                        out_node not in predecessors
                        and out_node.node_id != antichain_node
                    ):
                        extra_nodes.add(predecessor.node_id)
        self._augmented_antichains[antichain_key] = list(
            extra_nodes) + antichain
        return self._augmented_antichains[antichain_key]
    ...
    def deaugment_augmented_antichain(self, augmented_antichain):
        ...
        for augmented_antichain_node in augmented_antichain:
            if (
                augmented_antichain_node not in nodes_to_remove
                and augmented_antichain_node not in antichain
            ):
    ...
    def construct_antichain(self, augmented_antichain, old_node, new_node):
        new_antichain = [
            x if x != old_node else new_node for x in augmented_antichain]
    ...
    def next_antichains(self, antichain):
        ...
        for augmented_antichain_node in augmented_antichain:
            next_nodes = (
                self.edges[augmented_antichain_node]
                if augmented_antichain_node in self.edges
                else []
            )
            for next_node in next_nodes:
                ...
                if self.is_next_antichain(augmented_antichain, next_node.node_id):
                    next_antichain = self.construct_antichain(
                        augmented_antichain, augmented_antichain_node, next_node.node_id
                    )
    ...
    def antichain_dag(self):
        ...
        source_node = AntichainNode(
            "antichain_%d" % antichain_id, self.augment_antichain(antichain)
        )
        ...
        while len(antichain_queue) > 0:
            ...
            for next_antichain in next_antichains:
                ...
                if next_antichain_key not in antichain_mapping:
                    antichain_id += 1
                    next_antichain_node = AntichainNode(
                        "antichain_%d" % antichain_id,
                        self.augment_antichain(next_antichain),
                    )
                    antichain_mapping[next_antichain_key] = next_antichain_node
                antichain_dag.add_edge(
                    antichain_mapping[antichain_key],
    ...
    def from_str_bwd(graph_str):
        ...
        for graph_str_line in graph_str_lines:
            if not graph_str_line.startswith("\t"):
                ...
                if in_node_id not in gr.in_edges:
                    gr.in_edges[in_node_id] = [gr.nodes[node_id]]
                else:
                    gr.in_edges[in_node_id].append(gr.nodes[node_id])
                if node_id not in gr.edges:
                    gr.edges[node_id] = [gr.nodes[in_node_id]]
                else:
                    gr.edges[node_id].append(gr.nodes[in_node_id])
        return gr
    ...
    def to_dot(self, arch):
        ...
        for node in self.nodes.values():
            node_desc = (
                "%s\n[activation_size=%s; parameter_size=%s; output_act=%s; saved_tensors=%s; parameter=%s; kernel=%s; stride=%s; dilation=%s; padding=%s; contiguous=%s; fused_operators=%s]"
                % (
                    node.node_desc,
                    node.activation_size,
                    node.parameter_size,
                    node.output_act,
                    node.saved_tensors,
                    node.parameter,
            ...
            if node.stage_id is not None:
                ...
                dot.node(node.node_id, node_desc, color=color, style="filled")
            else:
    ...
    def plot_cdfs(self, cdfs, output_directory):
        ...
        for i in range(3):
            ...
            ax.plot(range(len(cdfs)), cdf, label=labels[i], linewidth=2)
        ax.set_xlim([0, None])
        ax.set_ylim([0, 100])
    ...
    def plot_bar_graph(
        self, all_values, ylabel, legend, output_template, output_directory
    ):
        import matplotlib
    ...
    def render_bar_graphs_and_cdfs(self, output_directory):
        ...
        for node in topological_ordering:
            ...
            if len(cdfs) == 0:
                ...
                cdfs.append(
                    [
                        cdfs[-1][0]
                        + node.forward_compute_time
                        + node.backward_compute_time,
        ...
        self.plot_bar_graph(pdfs, "PDF (\%)", False,
                            "%s_pdf.pdf", output_directory)
...
class Node(object):
    def __init__(
        ...
        self.depth = None
        self.height = None
        self.fwd_utilization = 0
        self.fwd_latency = 0
        self.fwd_comm_latency = 0
        self.fwd_energy = 0
        self.fwd_comm_energy = 0
        self.fwd_power = 0
        self.fwd_noc_bw = 0
        self.fwd_dram_bw = 0
        self.fwd_l1_size = 0
        self.fwd_l2_size = 0
        self.fwd_l2_ip_tile = 0
        self.bwd_utilization = 0
        self.bwd_latency = 0
        self.bwd_comm_latency = 0
        self.bwd_energy = 0
        self.bwd_comm_energy = 0
        self.bwd_power = 0
        self.bwd_noc_bw = 0
        self.bwd_dram_bw = 0
        self.bwd_l1_size = 0
        self.bwd_l2_size = 0
        self.bwd_l2_ip_tile = 0
        self.weight_update_utilization = 0
        self.weight_update_latency = 0
        self.weight_update_energy = 0
        self.weight_update_power = 0
        self.weight_update_noc_bw = 0
        self.weight_update_dram_bw = 0
        self.weight_update_l1_size = 0
        self.weight_update_l2_size = 0
        self.weight_update_l2_ip_tile = 0
        self.optimizer_latency = 0
        self.optimizer_energy = 0
        self.optimizer_dram_bw = 0
        self.sink_latency = 0
        self.asap_schd = None
        self.alap_schd = None
        self.schd_time = None
        self.mobility = None
        self.res_conflict = False
        self.res_conflict_delay = None
        self.fcf_done = False
        self.wcf_done = False
        self.core_id = None
    ...
    def __str__(self):
        stage_id_str = (
            " -- stage_id=%d" % self.stage_id if self.stage_id is not None else ""
        )
        node_desc = self.node_desc.replace("\n", "")
        activation_size = ("%s" % self.activation_size).replace(", ", "; ")
        return (
            "%s -- %s -- activation_size=%s; parameter_size=%s; output_act=%s; saved_tensors=%s; parameter=%s; kernel=%s; stride=%s; dilation=%s; padding=%s; contiguous=%s; fused_operators=%s%s"
            % (
                self.node_id,
                node_desc,
    ...
    def from_str(node_str):
        ...
        output_act = literal_eval(output_act.split("=")[1])
        saved_tensors = literal_eval(saved_tensors.split("=")[1])
        parameter = literal_eval(parameter.split("=")[1])
        kernel = literal_eval(kernel.split("=")[1])
        stride = literal_eval(stride.split("=")[1])
        dilation = literal_eval(dilation.split("=")[1])
        padding = literal_eval(padding.split("=")[1])
        contiguous = contiguous.split("=")[1]
        fused_operators = fused_operators.split("=")[1]
        return Node(
            node_id,
            node_desc,
