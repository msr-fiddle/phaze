    ...
    def gen_mask(self, image_size, mask_size, mask_type, patch_size):
        ...
        if mask_type == 'random':
            ...
            for i in range(mask_size):
                ...
                x = torch.clamp(
                    x + action_list[r][0], min=0, max=img_size_patch - 1)
                y = torch.clamp(
                    y + action_list[r][1], min=0, max=img_size_patch - 1)
                x_offset = x * patch_size
                y_offset = y * patch_size
                mask[x_offset:x_offset+patch_size,
                     y_offset:y_offset+patch_size] = 1
        else:
