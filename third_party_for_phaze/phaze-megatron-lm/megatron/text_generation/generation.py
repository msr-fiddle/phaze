...
def score_and_return_on_first_stage(model, tokens, lengths):
    ...
    if max_prompt_length * batch_size > args.max_tokens_to_oom:
        raise ValueError("Too many tokens.  " + str(max_prompt_length *
                         batch_size) + " is greater than "+str(args.max_tokens_to_oom))
...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    if max_sequence_length * batch_size > args.max_tokens_to_oom:
        raise ValueError("Too many tokens.  " + str(max_sequence_length *
                         batch_size) + " is greater than "+str(args.max_tokens_to_oom))
        ...
        for context_length in range(min_prompt_length, max_sequence_length):
            ...
            if mpu.is_pipeline_last_stage():
                if prevent_newline_after_colon:
                    # disable "\n" after ":"
                    logits[tokens2use[:, -1] ==
                           tokenizer.tokenize(':')[0], -1, tokenizer.tokenize('\n')[0]] = -1e10
                # Always the last stage should have an output.
                assert logits is not None
...
def beam_search_and_return_on_first_stage(model, tokens, lengths, beam_size, stop_token, num_return_gen, length_penalty, prevent_newline_after_colon=True):
    ...
    final_sequence_length = min(
        final_sequence_length, args.max_position_embeddings)
    ...
    with torch.no_grad():
        ...
        for context_length in range(prompt_length, final_sequence_length):
            ...
            if mpu.is_pipeline_last_stage():
                ...
                best_beam_ids = torch.div(
                    indices[: 2 * beam_size], vocab_size).trunc().long()
                ...
                if beam_hyp.is_done(best_scores.max().item(), context_length + 1 - prompt_length):
                    done = torch.ones(1, dtype=torch.uint8,
                                      device=torch.cuda.current_device())
        ...
        if mpu.is_pipeline_last_stage():
            ...
            if not done:
                for beam_id in range(beam_size):
                    beam_hyp.add(tokens[beam_id].clone(), scores[beam_id].squeeze(
                    ), context_length + 1 - prompt_length)

            # rank based on scores
            sorted_hyps = sorted(
                beam_hyp.beams, key=lambda x: x[0], reverse=True)
