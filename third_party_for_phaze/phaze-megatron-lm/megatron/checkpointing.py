...
def get_checkpoint_names(checkpoints_path, iteration, use_distributed_optimizer, release=False,
    ...
    if use_distributed_optimizer:
        ...
        model_name = optim_name = os.path.join(
            common_path, "model_optim_rng.pt")
    ...
    try:
        ...
        if use_distributed_optimizer:
            optim_state_dict = torch.load(
                optim_checkpoint_name, map_location='cpu')
        else:
...
def load_args_from_checkpoint(args, load_arg='load'):
    ...
    def _set_arg(arg_name, old_arg_name=None, force=False):
        ...
        if checkpoint_value is not None:
            print_rank_0(
                f"Setting {arg_name} to {checkpoint_value} from checkpoint")
